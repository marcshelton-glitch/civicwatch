      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 20, flexWrap: "wrap" }}>
        {tabs.map(t => (
          <button key={t.id} className={`rep-tab ${repTab === t.id ? "active" : ""}`}
            ...
            style={{ padding: "8px 14px", border: `1px solid ${S.border}`, borderRadius: 8, cursor: "pointer", fontFamily: "inherit", fontSize: 12, fontWeight: 600, background: repTab === t.id ? S.red : S.cardBg, color: repTab === t.id ? "white" : S.gray, transition: "all 0.2s" }}>
            {t.label}
          </button>
        ))}
      </div>
